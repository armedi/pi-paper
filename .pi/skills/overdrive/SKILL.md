...
Push an interface past conventional limits. Start by making the Paper design itself feel extraordinary through ambitious composition, motion-ready states, and visually distinctive craft. Only move into browser-specific implementation if the user explicitly asks for code export.
...
2. **ask the user directly to clarify what you cannot infer.** to present these directions and get the user's pick before writing any design or code. Explain trade-offs (complexity, export feasibility, performance cost if implemented).
3. Only proceed with the direction the user confirms.

Skipping this step risks building something embarrassing that needs to be thrown away.

### Iterate with Paper Review

Technically ambitious directions almost never work on the first try. You MUST actively use the Paper canvas and screenshot reviews to preview your work, visually verify the result, and iterate. Do not assume the effect looks right — check it. Expect multiple rounds of refinement. The gap between "technically works" and "looks extraordinary" is closed through visual iteration, not speculation.
...
Remember: "Extraordinary" isn't about using the newest API. It's about making an interface do something users didn't think this product could do, starting in the Paper design and extending into code only when asked.